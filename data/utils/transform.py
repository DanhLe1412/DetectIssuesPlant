...
def np_draw_gaussian(bbox, label, config):
    tl_heatmaps = np.zeros(
        (config.OUTPUT_SIZE[0], config.OUTPUT_SIZE[1], config.CLASSES), dtype=np.float32)
    br_heatmaps = np.zeros(
        (config.OUTPUT_SIZE[0], config.OUTPUT_SIZE[1], config.CLASSES), dtype=np.float32)
    ct_heatmaps = np.zeros(
        (config.OUTPUT_SIZE[0], config.OUTPUT_SIZE[1], config.CLASSES), dtype=np.float32)
    tl_regrs = np.zeros(
        (config.OUTPUT_SIZE[0], config.OUTPUT_SIZE[1], 2), dtype=np.float32)
    br_regrs = np.zeros(
        (config.OUTPUT_SIZE[0], config.OUTPUT_SIZE[1], 2), dtype=np.float32)
    ct_regrs = np.zeros(
        (config.OUTPUT_SIZE[0], config.OUTPUT_SIZE[1], 2), dtype=np.float32)
    mask = np.zeros(
        (3, config.OUTPUT_SIZE[0], config.OUTPUT_SIZE[1]), dtype=np.float32)
